...
//LRESULT __stdcall KeyboardProc(_In_ int nCode, _In_ WPARAM wParam, LPARAM lParam)
//{
//    //qDebug()<<nCode<<","<<wParam<<","<<lParam;
//    return CallNextHookEx(NULL, nCode, wParam, lParam);//返回给下一个钩子子程处理
//}
LRESULT CALLBACK HookShoot(_In_ int nCode, _In_ WPARAM wParam, LPARAM lParam) {
    MOUSEHOOKSTRUCT* mshook = (MOUSEHOOKSTRUCT*)lParam;
    lParam=MAKELPARAM(mshook->pt.x<0?0: mshook->pt.x, mshook->pt.y < 0 ? 0 : mshook->pt.y);
    switch (wParam)
    {
    case WM_DESTROY:    // 終了
        PostQuitMessage(0);
...
bool LAppDelegate::Initialize() 
{
    ...
    infile >> LAppDefine::RenderTargetWidth >> LAppDefine::RenderTargetHeight >> LAppDefine::RenderTargetBiasX >> LAppDefine::RenderTargetBiasY >> LAppDefine::RenderTargetSize;
    
    infile >> LAppDefine::ResourcesPath;
    infile >> LAppDefine::BackImageName;
    infile >> LAppDefine::GearImageName;
    infile >> LAppDefine::PowerImageName;
    vector<string> cnt;
    string temp;
    ...
    if (cnt.size())
    {
        LAppDefine::ModelDir = cnt;
        LAppDefine::ModelDirSize = cnt.size();
    }
    infile.close();

    // outfile << clientRect.left << " " << clientRect.top << endl;
    // outfile.close();
    if (DebugLogEnable)
    {
...
void LAppDelegate::GetClientSize(int& rWidth, int& rHeight)
{
    ...
    rWidth = LAppDefine::RenderTargetWidth;
    rHeight = LAppDefine::RenderTargetHeight;
    ...
    /*LAppPal::PrintLog("[APP]rWidth: [%d]", (clientRect.right - clientRect.left));
    LAppPal::PrintLog("[APP]rHeight: [%d]", (clientRect.bottom - clientRect.top));*/
}
