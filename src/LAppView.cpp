...
void LAppView::InitializeSprite()
{
    ...
    int width, height;
    LAppDelegate::GetInstance()->GetClientSize(width, height);
    width = LAppDefine::RenderTargetWidth;
    height = LAppDefine::RenderTargetHeight;
    ...
    y = static_cast<float>(50 + powerTexture->height * 0.5f);
...
void LAppView::OnTouchesEnded(float px, float py) const
{
    ...
    live2DManager->OnDrag(0.0f, 0.0f);
    {
        ...
        // 歯車にタップしたか
        if (_gear->IsHit(px, py) && !LAppDefine::MouseMove)
        {
            live2DManager->NextScene();
        }

        // 電源ボタンにタップしたか
        if (_power->IsHit(px, py) && !LAppDefine::MouseMove)
        {
